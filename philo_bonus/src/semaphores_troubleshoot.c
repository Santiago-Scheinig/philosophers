...
int	safe_sem_post(sem_t *sem, const char *sem_name, int id)
{
	...
	{
		if (id >= 0)
			printf("thd: sem_post failed on %s%i\n", sem_name, id);
		else
			printf("thd: sem_post failed on %s\n", sem_name);
		return (PH_SEM_PERR);
	}
	return (PH_SUCCESS);
}
...
int	safe_sem_wait(sem_t *sem, const char *sem_name, int id)
{
	while (sem_wait(sem) == -1)
	{
		...
		{
			if (id >= 0)
				printf("thd: sem_wait failed on %s%i\n", sem_name, id);
			else
				printf("thd: sem_wait failed on %s\n", sem_name);
			return (PH_SEM_WERR);
		}
	}
	return (PH_SUCCESS);
...
int	try_exit_and_kill(t_rules *table, int errcode)
{
	...
	if (safe_sem_wait(table->sem_philo[0], "/philo_", 0))
